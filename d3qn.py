...
#         self.n_bins = 3
#         self.memory_size = 32_768
#         self.batch_size = 1024
#         self.gamma = 0.99
#         self.eps_start = 1.0
#         self.eps_end = 0.05
#         self.eps_decay = 100_000
#         self.lr = 1e-4
...
#         batch_idxs = np.random.choice(len(self.buffer), batch_size, replace=False)
...
#     def select_action(self, state, policy_net, n_actions, steps_done, device):
...
#         self.config = config
#         os.makedirs("figures & d3qn", exist_ok=True)
    
#     def save_reward_plot(self, episode_rewards, episode):
#         plt.figure(figsize=self.config.figure_size, dpi=self.config.figure_dpi)
#         plt.plot(episode_rewards, label="Total Reward", linewidth=2, color='blue', alpha=0.7)
...
#                     label=f"Moving Average ({window_size})", linewidth=2, color='red', alpha=0.8)
        
#         plt.xlabel("Episode", fontsize=14)
#         plt.ylabel("Total Reward", fontsize=14)
#         plt.title(f"BipedalWalker-v3 D3QN Training — Episode {episode}", fontsize=16, fontweight='bold')
#         plt.legend(fontsize=12)
#         plt.grid(True, alpha=0.3)
#         plt.tight_layout()
        
#         filename = f"{self.config.path}/reward_plot_episode_{episode:04d}.png"
...
#         # Networks
#         self.policy_net = DuelingDQN(state_dim, action_space.n_actions).to(config.device)
#         self.target_net = DuelingDQN(state_dim, action_space.n_actions).to(config.device)
...
#         self.policy = EpsilonGreedyPolicy(config.eps_start, config.eps_end, config.eps_decay)
...
#         state_batch = torch.tensor(states, dtype=torch.float32, device=self.config.device)
#         action_batch = torch.tensor(actions, dtype=torch.int64, device=self.config.device)
#         reward_batch = torch.tensor(rewards, dtype=torch.float32, device=self.config.device)
#         next_state_batch = torch.tensor(next_states, dtype=torch.float32, device=self.config.device)
#         done_batch = torch.tensor(dones, dtype=torch.float32, device=self.config.device)
...
#         loss.backward()
#         # torch.nn.utils.clip_grad_norm_(self.policy_net.parameters(), 10)  # Gradient clipping
#         self.optimizer.step()
...
#             for step in tqdm(range(1, self.config.max_steps + 1), desc="Steps", leave=False):
#                 # Select and perform action
#                 action_idx = self.select_action(state, episode * step)
...
#             # Save plot periodically (starting from episode 100, then every 100 episodes)
#             if (episode + 1) % self.config.plot_freq == 0:
#                 self.plot_manager.save_reward_plot(episode_rewards, episode + 1)
#                 ep = episode + 1
#                 policy_path = f"{self.config.path}/checkpoints/policy_net_ep{ep:04d}.pth"
#                 torch.save({
#                     'episode': ep,
#                     'model_state_dict': self.policy_net.state_dict(),
#                     'optimizer_state_dict': self.optimizer.state_dict(),
#                     'epsilon': self.policy.eps_end + (self.policy.eps_start - self.policy.eps_end) * 
#                                np.exp(-1. * (episode * self.config.max_steps) / self.policy.eps_decay),
#                 }, policy_path)
#                 print(f"Checkpoint saved: {policy_path}")

#                 target_path = f"{self.config.path}/checkpoints/target_net_ep{ep:04d}.pth"
#                 torch.save(self.target_net.state_dict(), target_path)
#                 print(f"Target network saved: {target_path}")
...
#     agent = D3QNAgent(config, action_space, state_dim)
#     episode_rewards = agent.train(env)    
#     agent.plot_manager.save_reward_plot(episode_rewards, config.num_episodes)
#     env.close()
